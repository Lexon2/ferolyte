...
describe('BlockBuilder', () => {
  ...
  it('returns file name from identifier', () => {
    expect(new BlockBuilder(minimalBlockConfig({ identifier: 'test:my_block' })).fileName()).toBe('my_block.block.json');
  });

  it('uses custom version and menu category', () => {
    const block = new BlockBuilder(minimalBlockConfig({
      version: '1.21.90',
      menuCategory: { category: 'construction' },
    })).build();

    expect(block.format_version).toBe('1.21.90');
    expect(block['minecraft:block'].description.menu_category).toEqual({ category: 'construction' });
  });
  ...
  it('omits components section when conversion returns undefined', () => {
    const block = new BlockBuilder(minimalBlockConfig({ components: {} })).build();
  ...
  it('clones config independently', () => {
    const builder = new BlockBuilder(minimalBlockConfig({ components: { replaceable: true } }));
